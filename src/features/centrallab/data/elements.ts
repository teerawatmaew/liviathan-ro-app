import type { ElementTables } from '../types'
...
export function getBestAttackerElements(
  defenderElement: import('../types').ElementType,
  defenderLevel: import('../types').ElementLevel,
) {
  const row = elementTables[defenderLevel][defenderElement]
  return (Object.entries(row) as [import('../types').ElementType, number][])
    .sort((a, b) => b[1] - a[1])
}
